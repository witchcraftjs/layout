...
export function getCloseFrameInfo<T extends "edge" | "dir">(
	...
	KnownError<LAYOUT_ERROR.CANT_CLOSE_NEARBY_FRAMES_TOO_SMALL | LAYOUT_ERROR.CANT_CLOSE_NO_DRAG_EDGE | LAYOUT_ERROR.CANT_CLOSE_SINGLE_FRAME | LAYOUT_ERROR.CANT_CLOSE_WITHOUT_FORCE> {
	if (frames.length === 1) {
	...
	if (!entriesInVisualEdges) unreachable()
