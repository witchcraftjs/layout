...
[![Build](https://github.com/witchcraftjs/layout/actions/workflows/build.yml/badge.svg)](https://github.com/witchcraftjs/layout/actions/workflows/build.yml)
...
<!-- [![NPM Version (with beta tag)](https://img.shields.io/npm/v/%40alanscodelog%2FREPONAME/beta)](https://www.npmjs.com/package/@witchcraft/layout/v/beta) -->
...
Or if using zod, you can do something like this. Note that the `zLayoutWindow` and `zLayoutFrame` types have `.passthrough` to allow for extra properties.

```ts
import { zLayoutFrame } from "@witchcraft/layout"
import { z } from "zod"
import { layoutCreate } from "@witchcraft-layout/layout/index.js"
...
export const zAppFrame = z.discriminatedUnion("type", [
	...
	zFrameWoId.extend({
		...
		content: z.object({
			someOtherKey: z.optional(z.string().uuid()),
		}),
	}),
]).and(z.object({
	id: z.string().uuid(),
